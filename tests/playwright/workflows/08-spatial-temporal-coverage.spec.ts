...
test.describe('Spatial and Temporal Coverage', () => {
    ...
    test.describe('Temporal Input', () => {
        ...
        test('should allow multiple temporal-only entries without an implicit timezone', async ({ page }) => {
            const addButton = page.getByRole('button', { name: /add.*coverage entry/i }).first();
            await expect(addButton).toBeVisible();
            await addButton.click();

            const timezoneSelect = page.getByRole('combobox', { name: /timezone/i }).first();
            await expect(timezoneSelect).toContainText('Select');

            await page.locator('#end-date').first().fill('2026-08-27');
            const addAnotherButton = page.getByRole('button', { name: /add coverage entry/i }).last();
            await expect(addAnotherButton).toBeEnabled();
    ...
    test.describe('Description Field', () => {
        ...
        test('should not widen the editor when a long description is collapsed', async ({ page, browserName }) => {
            test.skip(
                browserName === 'webkit',
                'Skipped on WebKit; Issue #1095 was reported in Firefox and Chromium provides the cross-browser control.',
            );
    ...
    test.describe('Entry Management', () => {
        ...
        test('should allow removing non-first entries', async ({ page }) => {
            ...
            if (await firstAddButton.isVisible()) {
                ...
                // Add second entry
                await page
                    .getByRole('button', { name: /add coverage entry/i })
                    .last()
                    .click();
    ...
    test.describe('Complete Workflow', () => {
        test('should complete full coverage entry workflow', async ({ page }) => {
            ...
            if (await addButton.isVisible()) {
                ...
                await page
                    .getByPlaceholder(/deep drilling campaign/i)
                    .first()
                    .fill('Field campaign in Munich area');
