...
it('publishes a latest-release-only digest-pinned Production deployment branch', function (): void {
    ...
    expect($workflowContents)->toBeString()
        ...
        ->toContain('PUBLISHED_STABLE_TAGS="$(')
        ->toContain('done <<< "$PUBLISHED_STABLE_TAGS"')
        ->not->toContain('done < <(')
        ->toContain('CANDIDATE_RELEASE_LISTED=false')
        ->toContain('GitHub returned no published stable releases')
        ->toContain('is missing from GitHub\'s published stable release list')
