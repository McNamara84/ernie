...
it('stores descriptions and dates with a resource', function (): void {
    ...
    // Create required date types
    DateType::query()->create(['name' => 'Created', 'slug' => 'created']);
    DateType::query()->create(['name' => 'Collected', 'slug' => 'collected']);
    ...
    $payload = [
        ...
        'dates' => [
            // Note: 'created' is no longer sent - it's auto-managed by the backend
            [
                'dateType' => 'collected',
    ...
    $resource = Resource::query()
        ->with(['descriptions', 'dates.dateType'])
        ->firstWhere('doi', '10.1234/descriptions-dates');

    expect($resource)->not->toBeNull();
    expect($resource->descriptions)->toHaveCount(2);
    // 2 dates: 1 auto-generated 'created' + 1 user-provided 'collected'
    ...
    // Check dates - 'created' is auto-generated by the backend
    $created = $resource->dates->first(fn ($d) => $d->dateType?->slug === 'created');
    expect($created)->not->toBeNull();
    expect($created?->start_date)->not->toBeNull(); // Auto-generated with current timestamp

    $collected = $resource->dates->first(fn ($d) => $d->dateType?->slug === 'collected');
...
it('updates descriptions and dates when updating a resource', function (): void {
    ...
    // Create required date types
    $createdDateType = DateType::query()->create(['name' => 'Created', 'slug' => 'created']);
    DateType::query()->create(['name' => 'Updated', 'slug' => 'updated']);
    DateType::query()->create(['name' => 'Collected', 'slug' => 'collected']);
    ...
    // Original 'created' date - this should be preserved on update
    $resource->dates()->create([
        'date_type_id' => $createdDateType->id,
    ...
    $payload = [
        ...
        // Note: 'created' and 'updated' dates are auto-managed, not sent in payload
        'dates' => [
            [
                'dateType' => 'collected',
    ...
    $resource->refresh();
    $resource->load(['descriptions', 'dates.dateType']);

    // Verify old descriptions and dates were deleted
    expect($resource->descriptions)->toHaveCount(2);
    // 3 dates: preserved 'created' + auto-added 'updated' + user-provided 'collected'
    expect($resource->dates)->toHaveCount(3);
    ...
    // Verify dates - 'created' is preserved from original
    $created = $resource->dates->first(fn ($d) => $d->dateType?->slug === 'created');
    expect($created?->start_date?->toDateString())->toBe('2024-01-01'); // Original date preserved

    // 'updated' is auto-added on update
    $updated = $resource->dates->first(fn ($d) => $d->dateType?->slug === 'updated');
    expect($updated)->not->toBeNull();
    expect($updated?->start_date)->not->toBeNull();

    // User-provided 'collected' date
    $collected = $resource->dates->first(fn ($d) => $d->dateType?->slug === 'collected');
    expect($collected?->start_date?->toDateString())->toBe('2024-02-15');
});
