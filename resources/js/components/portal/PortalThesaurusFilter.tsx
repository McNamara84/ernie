...
function ThesaurusTreeNode({ node, level = 0, expandedAncestorIds, selectedNodeIdSet, onToggleNode }: ThesaurusTreeNodeProps) {
    ...
    const shouldExpand = expandedAncestorIds.has(node.id);
...
export function PortalThesaurusFilter({
    facets = [],
    selectedNodeIds = [],
    onSelectionChange = noopSelectionChange,
    hideTitle = false,
}: PortalThesaurusFilterProps) {
    ...
    const { expandedAncestorIds, selectedNodes } = useMemo(() => {
        ...
        <div className="space-y-3">
            ...
            {facets.length === 0 ? (
                ...
                <div className="rounded-lg border bg-background/80">
                    <ul role="tree" aria-label="Thesaurus keyword hierarchies" aria-multiselectable={true} className="py-1">
                        {facets.flatMap((facet) =>
                            facet.roots.map((root) => (
                                <ThesaurusTreeNode
                                    key={`${facet.scheme}-${root.id}`}
                                    node={root}
                                    expandedAncestorIds={expandedAncestorIds}
                                    selectedNodeIdSet={selectedNodeIdSet}
