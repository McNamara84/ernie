...
/** Delay before reloading page after successful PID update (in ms) */
const UPDATE_SUCCESS_RELOAD_DELAY_MS = 1500;

export interface PidSettingData {
    type: string;
    displayName: string;
    isActive: boolean;
...
function PidSettingRow({ pidSetting, onActiveChange, onElmoActiveChange, onUpdateComplete }: PidSettingRowProps) {
    ...
    const checkForUpdates = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/pid-settings/${pidSetting.type}/check`, {
                method: 'POST',
                headers: {
    ...
    const pollJobStatus = useCallback(
        async (jobId: string) => {
            try {
                const response = await fetch(`/pid-settings/update-status/${jobId}`, {
                    credentials: 'include',
                });
    ...
    const triggerUpdate = useCallback(async () => {
        try {
            const response = await fetch(`/pid-settings/${pidSetting.type}/update`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-XSRF-TOKEN': getCsrfToken(),
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex gap-4">
                    <div className="flex items-center gap-2">
                        <Checkbox
                            id={`pid-ernie-${pidSetting.type}`}
                            checked={pidSetting.isActive}
                            onCheckedChange={(checked) => onActiveChange(pidSetting.type, checked === true)}
                        />
                        <Label htmlFor={`pid-ernie-${pidSetting.type}`} className="text-sm font-normal">
                            ERNIE
                        </Label>
                    </div>
                    <div className="flex items-center gap-2">
                        <Checkbox
                            id={`pid-elmo-${pidSetting.type}`}
                            checked={pidSetting.isElmoActive}
                            onCheckedChange={(checked) => onElmoActiveChange(pidSetting.type, checked === true)}
                        />
                        <Label htmlFor={`pid-elmo-${pidSetting.type}`} className="text-sm font-normal">
                            ELMO
                        </Label>
                    </div>
                </div>
...
export interface PidSettingsCardProps {
...
export function PidSettingsCard({
    ...
    const handleUpdateComplete = useCallback(() => {
        setTimeout(() => {
            router.reload({ only: ['pidSettings'] });
