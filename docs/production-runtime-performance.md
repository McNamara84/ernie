...
The workers process one resource per job and share a Redis-backed limiter. The conservative defaults are concurrency `1`, at most `80` request starts per rolling minute, and at least `750` ms between starts. `FUJI_TIMEOUT=300` accommodates slow evaluations, `FUJI_ASSESSMENT_ITEM_TIMEOUT=330` remains above that HTTP timeout, and `FUJI_ASSESSMENT_LEASE_SECONDS=390` plus `FUJI_ASSESSMENT_QUEUE_RETRY_AFTER=390` remain above the item timeout. Validate F-UJI latency, 429 responses, errors, CPU, and memory on Stage before increasing concurrency. On a host that exposes only one logical CPU, keep `FUJI_ASSESSMENT_CONCURRENCY=1` in the Portainer stack environment so the persisted run configuration and deployed worker count remain consistent.

ERNIE distinguishes resource failures from assessment-service errors. HTTP 4xx responses other than 429 are resource failures. DNS and connection failures, HTTP 429, HTTP 5xx, and invalid service payloads are service errors. Short connection failures and retryable HTTP responses retain bounded retries; a cURL 28 response timeout that already consumed the complete evaluation window is not repeated immediately. Each failed run item stores its error code, sanitized underlying detail, HTTP status, and attempt duration. Completed runs with service errors expose `Retry service errors`, which queues only those items and preserves successful results and resource failures.
