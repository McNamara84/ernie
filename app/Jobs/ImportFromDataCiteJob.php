...
class ImportFromDataCiteJob implements ShouldQueue
{
        ...
        try {
            ...
            foreach ($importService->fetchAllDois() as $doiRecord) {
                ...
                try {
                    ...
                    try {
                        $fileUrls = $metaworksService->lookupFileUrls($doi);

                        if ($fileUrls !== []) {
                            $this->createLandingPageWithFiles($importedResource, $fileUrls);
                        }
                    } catch (\Throwable $e) {
     ...
     *
     * The landing page is created as published with the default_gfz template.
     * The slug and DOI prefix are auto-generated by the LandingPage model's boot event.
     *
     ...
     */
    private function createLandingPageWithFiles(Resource $resource, array $fileUrls): void
    {
        // Guard against duplicate landing pages (should not happen per decision #9,
        // but protects against re-runs or race conditions)
