...
class DataCiteXmlExporter
{
    ...
    private function buildSubjects(Resource $resource): void
    {
        ...
        foreach ($resource->subjects as $subjectModel) {
            $subject = $this->dom->createElement('subject', htmlspecialchars($this->dataCiteSubjectValue($subjectModel)));
            $language = trim((string) $subjectModel->language);

            $subject->setAttributeNS(self::XML_NAMESPACE, 'xml:lang', $language !== '' ? $language : 'en');
    ...
    private function buildContributors(Resource $resource): void
    {
        ...
        $fallbackOtherType = ContributorType::where('slug', 'Other')->first();
        ...
        foreach ($resource->contributors as $contributor) {
            ...
            if ($types->isEmpty()) {
                $types = $fallbackOtherType ? collect([$fallbackOtherType]) : collect();
            }
    ...
    private function buildAlternateIdentifiers(Resource $resource): void
    {
        ...
        foreach ($resource->alternateIdentifiers as $altId) {
            $alternateIdentifier = $this->dom->createElement(
                'alternateIdentifier',
                htmlspecialchars($altId->value)
            );
            $alternateIdentifier->setAttribute('alternateIdentifierType', $altId->type);
    ...
    private function buildGeoLocations(Resource $resource): void
    {
        ...
        foreach ($resource->geoLocations as $geoLocation) {
            ...
            if ($geoLocation->hasPolygon() && ! empty($geoLocation->polygon_points) && count($geoLocation->polygon_points) >= 3) {
            ...
            if ($geoLocation->hasLine() && ! empty($geoLocation->polygon_points) && count($geoLocation->polygon_points) >= 2) {
                $convertedPoints = $this->convertLineToPolygonPoints($geoLocation->polygon_points);
                $geoLocationPolygon = $this->dom->createElement('geoLocationPolygon');

                foreach ($convertedPoints as $point) {
                    $polygonPointElement = $this->dom->createElement('polygonPoint');
