...
final class IgsnClassificationBackfillService
{
    ...
    public function run(
        ...
        while ($limit === 0 || $stats['scanned'] < $limit) {
            ...
            $resources = Resource::query()
                ->with(['igsnMetadata', 'igsnClassifications', 'landingPage'])
    ...
    private function updateResource(Resource $resource, array $fields, bool $apply): array
    {
        ...
        if ($apply && $changed) {
            DB::transaction(function () use ($resource, $toInsert, $toFillType): void {
                ...
                foreach ($toFillType as $update) {
                    $update['classification']->classification_type = $update['classification_type'];
                    $update['classification']->save();
                }
            });
